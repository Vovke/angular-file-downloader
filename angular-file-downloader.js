 ...
 * Credits for the mimetypeConverter part to https://github.com/rsdoiel/mimetype-js
 */
angular.module('angularFileDownloader', [])
    .factory('angularFileDownloader', function($http) {
        var mimeTypeConverter = function (fileName){
            var MimeType = {
                charset: 'UTF-8',
                default_mime_type: 'text/plain',
                catalog: {},
                lookup: function (fname, include_charset, default_mime_type) {
                    default_mime_type =  default_mime_type || 'text/plain';
        ...
        var angularElementFileResolve = function(response){
            var file = new Blob([ response.data ], {type: mimeTypeConverter(fileName)});
